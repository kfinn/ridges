...
pub const Repo = ridges_lib.Repo(@This(), .{ .Validator = struct {
    pub fn validate(self: anytype, allocator: std.mem.Allocator) !ridges_lib.validation.RecordErrors(@TypeOf(self)) {
        var errors = ridges_lib.validation.RecordErrors(@TypeOf(self)).init(allocator);
